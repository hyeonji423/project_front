...
const Mypage = ({ user, onClose, onLogout }) => {
  ...
  if (!user) {
    return (
      <div
        ref={modalRef}
        className="absolute right-0 top-full w-60 bg-white rounded-lg p-4 z-60 webkit-z-60 shadow-[0_2px_6px_rgba(0,0,0,0.05),0_-2px_6px_rgba(0,0,0,0.05)]"
      >
  ...
  return (
    <div
      ref={modalRef}
      className="absolute right-0 top-full w-60 bg-white rounded-lg p-4 z-50 shadow-[0_2px_6px_rgba(0,0,0,0.05),0_-2px_6px_rgba(0,0,0,0.05)]"
    >
      <div className="flex justify-end">
        <IoClose
          onClick={onClose}
      ...
      <div className="flex flex-col items-center mb-3">
        ...
        <h3 className="tracking-tight text-base font-bold mt-4">
          {user?.email}
        </h3>
      </div>
      ...
      <ul className="space-y-1 md:space-y-2 text-sm md:text-base">
        <Link to="/mymedi_list" onClick={onClose}>
          <li className="p-1 hover:bg-blue-50 rounded mb-1 text-sm sm:text-base">
            약품 관리
          </li>
        </Link>
        ...
        <Link onClick={handleLogout} to="/">
          <li className="p-1 hover:bg-blue-50 rounded flex items-center justify-between mb-1 text-sm sm:text-base">
            로그아웃
            <LuLogOut />
          </li>
        </Link>
